  ...
  tabsetPanel(type = "tabs",
              ...
              tabPanel("Explore Topics",
                       ...
                       fluidPage(
                         ...
                         fluidRow(
                           radioButtons("TModels",
                                        "Which topics graph would you like to view? (Due to some browser incompatibilities,
                                        you may have to click back and forth between timeline and posts per topic in 
                                        order to see either of those graphs).",
                                        choiceNames = list("General topic model", "Topics timeline", "Posts per Topic"),
                                        choiceValues = list("tm", "tl", "ppt"),
                                        selected = "tm",
                                        inline = TRUE)
                         ),
                         br(),
                         fluidRow(
#                           includeHTML("Data/topics_final_LDAn20.html")
                           uiOutput("TopicModels")
                           ),
                         br(),
...
###### CREATE PLOTS  for Topic Models ######---------------------------------------------   
  
  output$TopicModels <- renderUI({
    t <- req(input$TModels)
    fName <- switch(t,
                    "tm" = "topics_final_LDAn20.html",
                    "tl" = "topics_timeline.html",
                    "ppt" = "Posts_per_Topic.html")
